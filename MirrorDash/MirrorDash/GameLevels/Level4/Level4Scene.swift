...
class level4Scene: SKScene, SKPhysicsContactDelegate {
    ...
    private var shieldIsActive = false
    ...
    private func setupScene() {
        ...
        self.shield = leftline.childNode(withName: "shield") as? SKSpriteNode
    ...
    private func checkCollisionWithShield() {
        let shieldWorldFrame = shield.parent?.convert(shield.position, to: self) ?? shield.position
        if pinkBall.frame.contains(shieldWorldFrame) {
            NotificationCenter.default.post(
                name: NSNotification.Name("ShieldIsActive"),
                object: nil,
    ...
    private func addShieldEffect() {
        ...
        let borderColor = SKColor.white

        blueBall.run(SKAction.run { self.blueBall.addBorder(width: borderWidth, color: borderColor) })
        pinkBall.run(SKAction.run { self.pinkBall.addBorder(width: borderWidth, color: borderColor) })
    }

    private func removeShieldEffect() {
        blueBall.run(SKAction.run { self.blueBall.removeBorder() })
        pinkBall.run(SKAction.run { self.pinkBall.removeBorder() })
    }
    ...
    private func showFinishLevelPopUp(result: GameResult) {
        saveTotalCoins()
        if result == .success {
            UserDefaults.standard.set(4, forKey: "finishedLevel")
        }
...
extension SKSpriteNode {
    func addBorder(width: CGFloat, color: SKColor) {
        let shapeNode = SKShapeNode(circleOfRadius: self.size.width + 5)
        shapeNode.strokeColor = color
        shapeNode.lineWidth = width
        shapeNode.zPosition = self.zPosition + 1
